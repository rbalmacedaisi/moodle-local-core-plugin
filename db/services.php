<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * Grupo Makro Core is a plugin used by the various components developed for the Grupo Makro platform.
 *
 * @package    local_grupomakro_core
 * @copyright  2022 Solutto Consulting <devs@soluttoconsulting.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

defined('MOODLE_INTERNAL') || die();

$functions = array(
    'local_grupomakro_create_user' => array(
        'classname' => 'local_grupomakro_core\external\create_user',
        'methodname' => 'execute',
        'description' => 'Ths method creates a new user in the Moodle platform.',
        'capabilities'  => 'local/grupomakro_core:manage_users',
        'type' => 'write',
        "ajax" => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_user_status' => array(
        'classname' => 'local_grupomakro_core\external\get_user_status',
        'methodname' => 'execute',
        'description' => 'Returns the status of the user in the Moodle platform.',
        'capabilities'  => 'local/grupomakro_core:manage_users',
        'type' => 'read',
        "ajax" => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_create_class' => array(
        'classname'     => 'local_grupomakro_core\external\gmkclass\create_class',
        'methodname'    => 'execute',
        'description'   => 'Create new class',
        'capabilities'  => 'local/grupomakro_core:manage_classes',
        'type'          => 'write',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_list_classes' => array(
        'classname'     => 'local_grupomakro_core\external\gmkclass\list_classes',
        'methodname'    => 'execute',
        'description'   => 'This method list the created classes',
        'capabilities'  => 'local/grupomakro_core:view_classmanagement',
        'type'          => 'read',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_delete_class' => array(
        'classname'     => 'local_grupomakro_core\external\gmkclass\delete_class',
        'methodname'    => 'execute',
        'description'   => 'This method delete the class with the id provided',
        'capabilities'  => 'local/grupomakro_core:manage_classes',
        'type'          => 'write',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_update_class' => array(
        'classname'     => 'local_grupomakro_core\external\gmkclass\update_class',
        'methodname'    => 'execute',
        'description'   => 'This method update the class with the id provided',
        'capabilities'  => 'local/grupomakro_core:manage_classes',
        'type'          => 'write',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_potential_class_teachers' => array(
        'classname'     => 'local_grupomakro_core\external\gmkclass\get_potential_class_teachers',
        'methodname'    => 'execute',
        'description'   => 'This method get the potential class teachers',
        'capabilities'  => 'local/grupomakro_core:manage_teachers',
        'type'          => 'read',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_teacher_available_courses' => array(
        'classname' => 'local_grupomakro_core\external\gmkclass\get_teacher_available_courses',
        'methodname' => 'execute',
        'description' => 'Get the teacher learning plan period available courses according to his/her skills',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:manage_teachers',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_teacher_available_days' => array(
        'classname' => 'local_grupomakro_core\external\gmkclass\get_teacher_available_days',
        'methodname' => 'execute',
        'description' => 'Get the teacher available days for the selected class time range',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:manage_teachers',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_calendar_get_calendar_events' => array(
        'classname' => 'local_grupomakro_core\external\event\get_calendar_events',
        'methodname' => 'execute',
        'description' => 'Get calendar events',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_course_announcements' => array(
        'classname' => 'local_grupomakro_core\external\event\get_course_announcements',
        'methodname' => 'execute',
        'description' => 'Get announcements (news forum posts) for a course',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_student_gradebook' => array(
        'classname' => 'local_grupomakro_core\external\student\get_student_gradebook',
        'methodname' => 'execute',
        'description' => 'Get structured gradebook for a student in a course',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_send_reschedule_message' => array(
        'classname' => 'local_grupomakro_core\external\activity\send_reschedule_message',
        'methodname' => 'execute',
        'description' => '',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:manage_classes',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_teachers_disponibility' => array(
        'classname' => 'local_grupomakro_core\external\disponibility\get_teachers_disponibility',
        'methodname' => 'execute',
        'description' => 'Get the teachers disponibility',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:manage_teacher_availability',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_add_teacher_disponibility' => array(
        'classname' => 'local_grupomakro_core\external\disponibility\add_teacher_disponibility',
        'methodname' => 'execute',
        'description' => 'Add teacher disponibility record',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_teacher_availability',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_update_teacher_disponibility' => array(
        'classname' => 'local_grupomakro_core\external\disponibility\update_teacher_disponibility',
        'methodname' => 'execute',
        'description' => 'Update teacher disponibility record',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_teacher_availability',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_delete_teacher_disponibility' => array(
        'classname' => 'local_grupomakro_core\external\disponibility\delete_teacher_disponibility',
        'methodname' => 'execute',
        'description' => 'Delete a teacher disponibility record',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_teacher_availability',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_teachers_disponibility_calendar' => array(
        'classname' => 'local_grupomakro_core\external\disponibility\get_teachers_disponibility_calendar',
        'methodname' => 'execute',
        'description' => 'Get the theachers disponibility for the disponibility calendar',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:manage_teacher_availability',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_check_reschedule_conflicts' => array(
        'classname' => 'local_grupomakro_core\external\activity\check_reschedule_conflicts',
        'methodname' => 'execute',
        'description' => 'Check if the new date and time for the activity is in conflict with another class for the users in the update activity',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:manage_classes',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_reschedule_activity' => array(
        'classname' => 'local_grupomakro_core\external\activity\reschedule_activity',
        'methodname' => 'execute',
        'description' => 'Reschedule an activity to another date and time',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_classes',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_check_copy_conflicts' => array(
        'classname' => 'local_grupomakro_core\external\activity\check_copy_conflicts',
        'methodname' => 'execute',
        'description' => 'Check schedule conflicts for each target date when copying a session',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:manage_classes',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_copy_activity' => array(
        'classname' => 'local_grupomakro_core\external\activity\copy_activity',
        'methodname' => 'execute',
        'description' => 'Copy an attendance session to one or more target dates (additive)',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_classes',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_delete_session' => array(
        'classname' => 'local_grupomakro_core\external\activity\delete_session',
        'methodname' => 'execute',
        'description' => 'Delete an attendance session and its associated BBB (refuses if attendance_log exists unless force=1)',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_classes',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_create_institution' => array(
        'classname' => 'local_grupomakro_core\external\institution\create_institution',
        'methodname' => 'execute',
        'description' => 'Create a new institution',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_institutions',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_update_institution' => array(
        'classname' => 'local_grupomakro_core\external\institution\update_institution',
        'methodname' => 'execute',
        'description' => 'Update an institution',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_institutions',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_list_institutions' => array(
        'classname' => 'local_grupomakro_core\external\institution\list_institutions',
        'methodname' => 'execute',
        'description' => 'Update an institution',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:manage_institutions',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_delete_institution' => array(
        'classname' => 'local_grupomakro_core\external\institution\delete_institution',
        'methodname' => 'execute',
        'description' => 'Update an institution',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_institutions',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_create_institution_contract' => array(
        'classname' => 'local_grupomakro_core\external\institutionContract\create_institution_contract',
        'methodname' => 'execute',
        'description' => 'Update an institution',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_institutional_contracts',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_update_institution_contract' => array(
        'classname' => 'local_grupomakro_core\external\institutionContract\update_institution_contract',
        'methodname' => 'execute',
        'description' => 'Update an institution',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_institutional_contracts',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_delete_institution_contract' => array(
        'classname' => 'local_grupomakro_core\external\institutionContract\delete_institution_contract',
        'methodname' => 'execute',
        'description' => 'Update an institution',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_institutional_contracts',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_list_institution_contracts' => array(
        'classname' => 'local_grupomakro_core\external\institutionContract\list_institution_contracts',
        'methodname' => 'execute',
        'description' => 'Update an institution',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:manage_institutional_contracts',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_create_contract_user' => array(
        'classname' => 'local_grupomakro_core\external\contractUser\create_contract_user',
        'methodname' => 'execute',
        'description' => 'Update an institution',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_institutional_contracts',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_update_contract_user' => array(
        'classname' => 'local_grupomakro_core\external\contractUser\update_contract_user',
        'methodname' => 'execute',
        'description' => 'Update an institution',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_institutional_contracts',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_delete_contract_user' => array(
        'classname' => 'local_grupomakro_core\external\contractUser\delete_contract_user',
        'methodname' => 'execute',
        'description' => 'Delete a contract user record',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_institutional_contracts',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_list_contract_users' => array(
        'classname' => 'local_grupomakro_core\external\contractUser\list_contract_users',
        'methodname' => 'execute',
        'description' => 'Update an institution',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:manage_institutional_contracts',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_generate_contract_enrol_link' => array(
        'classname' => 'local_grupomakro_core\external\contractUser\generate_contract_enrol_link',
        'methodname' => 'execute',
        'description' => 'generate a link for the users to enrol in a course related to a contract',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_institutional_contracts',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_create_student_user' => array(
        'classname' => 'local_grupomakro_core\external\contractUser\create_student_user',
        'methodname' => 'execute',
        'description' => 'Create a user and enrol in a course contract',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_institutional_contracts',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_bulk_create_contract_user' => array(
        'classname' => 'local_grupomakro_core\external\contractUser\bulk_create_contract_user',
        'methodname' => 'execute',
        'description' => 'Create a user and enrol in a course contract',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_institutional_contracts',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_student_active_classes' => array(
        'classname' => 'local_grupomakro_core\external\student\get_student_active_classes',
        'methodname' => 'execute',
        'description' => 'Get the student active classes',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:view_classmanagement',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_student_class_enrol' => array(
        'classname' => 'local_grupomakro_core\external\student\student_class_enrol',
        'methodname' => 'execute',
        'description' => 'Enrol a student in a class group',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:bulk_enroll',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_student_learning_plans_overview' => array(
        'classname' => 'local_grupomakro_core\external\student\get_student_learning_plans_overview',
        'methodname' => 'execute',
        'description' => 'Gets the student learning plans overview',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_student_learning_plan_pensum' => array(
        'classname' => 'local_grupomakro_core\external\student\get_student_learning_plan_pensum',
        'methodname' => 'execute',
        'description' => 'Gets the student learning plans pensum',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_student_course_pensum_activities' => array(
        'classname' => 'local_grupomakro_core\external\student\get_student_course_pensum_activities',
        'methodname' => 'execute',
        'description' => 'Gets the student course pensum activities',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),

    'local_grupomakro_approve_course_class_schedules' => array(
        'classname' => 'local_grupomakro_core\external\schedule\approve_course_class_schedules',
        'methodname' => 'execute',
        'description' => 'Approve a class schedule and enrol all the students in pre-registry and queue tables in its group',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_schedules',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_change_students_schedules' => array(
        'classname' => 'local_grupomakro_core\external\schedule\change_students_schedules',
        'methodname' => 'execute',
        'description' => 'Change the student(s) schedules from one class to another, either but puting them in the class pre-registry table or if full in the class queue table',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_schedules',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_delete_course_class_schedule' => array(
        'classname' => 'local_grupomakro_core\external\schedule\delete_course_class_schedule',
        'methodname' => 'execute',
        'description' => 'Delete a class and all the linked records from pre-registry and queue tables, save a message with the reason',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_schedules',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_delete_student_from_class_schedule' => array(
        'classname' => 'local_grupomakro_core\external\schedule\delete_student_from_class_schedule',
        'methodname' => 'execute',
        'description' => 'Delete a student from class pre-registry table or queue table otherwise',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_schedules',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_class_schedules_queues' => array(
        'classname' => 'local_grupomakro_core\external\schedule\get_class_schedules_queues',
        'methodname' => 'execute',
        'description' => 'Get the students info from que course classes schedule queue',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:view_classmanagement',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_course_class_schedules' => array(
        'classname' => 'local_grupomakro_core\external\schedule\get_course_class_schedules',
        'methodname' => 'execute',
        'description' => 'Get the course class schedules for an specific course and period',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:view_classmanagement',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_course_class_schedules_overview' => array(
        'classname' => 'local_grupomakro_core\external\schedule\get_course_class_schedules_overview',
        'methodname' => 'execute',
        'description' => 'Get the course class schedules for the created courses and periods',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:view_classmanagement',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_course_students_by_class_schedule' => array(
        'classname' => 'local_grupomakro_core\external\schedule\get_course_students_by_class_schedule',
        'methodname' => 'execute',
        'description' => 'Get the students from both pre-registry table and queue table for an specific class',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:view_classmanagement',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_scheduleless_students' => array(
        'classname' => 'local_grupomakro_core\external\schedule\get_scheduleless_students',
        'methodname' => 'execute',
        'description' => 'Get the students without schedule for an specific course',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:view_classmanagement',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_bulk_update_teachers_disponibility' => array(
        'classname' => 'local_grupomakro_core\external\disponibility\bulk_update_teachers_disponibility',
        'methodname' => 'execute',
        'description' => 'Bulk update teacher disponibilities',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_teacher_availability',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_close_current_period' => array(
        'classname' => 'local_grupomakro_core\external\period\close_current_period',
        'methodname' => 'execute',
        'description' => 'Close current period and update the learning plan users periods and courses',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_academic_planning',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_learning_plan_list' => array(
        'classname' => 'local_grupomakro_core\external\learningPlan\get_learning_plan_list',
        'methodname' => 'execute',
        'description' => 'Get the learning plans list for the moodle administrative panel',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_learning_plan_pensum' => array(
        'classname' => 'local_grupomakro_core\external\learningPlan\get_learning_plan_pensum',
        'methodname' => 'execute',
        'description' => 'Get the learning plan pensum',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_student_info' => array(
        'classname' => 'local_grupomakro_core\external\student\get_student_info',
        'methodname' => 'execute',
        'description' => 'Get information about all students',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_student_class_revalid_enrol' => array(
        'classname' => 'local_grupomakro_core\external\student\student_class_revalid_enrol',
        'methodname' => 'execute',
        'description' => 'Enrol a student in the corresponding revalid group',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:enrol_from_failed_subjects_report',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_student_get_revalids' => array(
        'classname' => 'local_grupomakro_core\external\student\student_get_revalids',
        'methodname' => 'execute',
        'description' => 'Get the active revalids activities for an user',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_upload_academic_calendar_period' => array(
        'classname' => 'local_grupomakro_core\external\period\upload_academic_calendar_period',
        'methodname' => 'execute',
        'description' => 'Upload an academic calendar period',
        'type' => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_academic_calendar',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_academic_calendar_period' => array(
        'classname' => 'local_grupomakro_core\external\period\get_academic_calendar_period',
        'methodname' => 'execute',
        'description' => 'Get an academic calendar period',
        'type' => 'read',
        'capabilities'  => 'local/grupomakro_core:view_academic_demand_gaps',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_bbb_module_url' => array(
        'classname' => 'local_grupomakro_core\external\activity\get_bbb_module_url',
        'methodname' => 'execute',
        'description' => 'Get the URL of the BBB session or recording',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_mark_bbb_attendance_by_playback' => array(
        'classname' => 'local_grupomakro_core\external\activity\mark_bbb_attendance_by_playback',
        'methodname' => 'execute',
        'description' => 'Get the URL of the BBB session or recording',
        'type' => 'write',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_logo_theme' => array(
        'classname' => 'local_grupomakro_core\external\themeSettings\get_logo_theme',
        'methodname' => 'execute',
        'description' => 'Get the theme settings and logo',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_learning_plans_data' => array(
        'classname' => 'local_grupomakro_core\external\learningPlan\get_learning_plans_data',
        'methodname' => 'execute',
        'description' => 'Get learning plans data with changes required by ISI',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_user_courses' => array(
        'classname' => 'local_grupomakro_core\external\student\get_user_courses',
        'methodname' => 'execute',
        'description' => 'Get user courses with changes required by ISI',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_user_courses_by_category' => array(
        'classname' => 'local_grupomakro_core\external\student\get_user_courses_by_category',
        'methodname' => 'execute',
        'description' => 'Get user courses by category with changes required by ISI',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_student_modules' => array(
        'classname' => 'local_grupomakro_core\external\student\get_student_modules',
        'methodname' => 'execute',
        'description' => 'Get active independent study module enrollments for a student',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_data_by_courses' => array(
        'classname' => 'local_grupomakro_core\external\student\get_data_by_courses',
        'methodname' => 'execute',
        'description' => 'Get data of courses with changes required by ISI',
        'type' => 'read',
        'capabilities' => '',
        'ajax'          => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_forum_activity_data' => array(
        'classname' => 'local_grupomakro_core\external\student\forum_activity',
        'methodname' => 'get_activity_data',
        'description' => 'Get forum discussions data for student interface',
        'type' => 'read',
        'capabilities' => '',
        'ajax' => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_forum_discussion_posts' => array(
        'classname' => 'local_grupomakro_core\external\student\forum_activity',
        'methodname' => 'get_discussion_posts',
        'description' => 'Get posts for a forum discussion',
        'type' => 'read',
        'capabilities' => '',
        'ajax' => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_create_forum_discussion' => array(
        'classname' => 'local_grupomakro_core\external\student\forum_activity',
        'methodname' => 'create_discussion',
        'description' => 'Create a forum discussion from student interface',
        'type' => 'write',
        'capabilities' => '',
        'ajax' => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_create_forum_reply' => array(
        'classname' => 'local_grupomakro_core\external\student\forum_activity',
        'methodname' => 'create_reply',
        'description' => 'Create a forum reply from student interface',
        'type' => 'write',
        'capabilities' => '',
        'ajax' => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_assign_activity_data' => array(
        'classname' => 'local_grupomakro_core\external\student\assign_activity',
        'methodname' => 'get_activity_data',
        'description' => 'Get assignment details and current student submission data',
        'type' => 'read',
        'capabilities' => '',
        'ajax' => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_submit_assign_activity' => array(
        'classname' => 'local_grupomakro_core\external\student\assign_activity',
        'methodname' => 'submit_activity',
        'description' => 'Submit assignment online text and files from simplified student UI',
        'type' => 'write',
        'capabilities' => '',
        'ajax' => true,
        'services' => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_withdraw_from_course' => array(
        'classname'   => 'local_grupomakro_core\external\schedule\withdraw_from_course',
        'methodname'  => 'execute',
        'description' => 'Fully withdraw a student from a Moodle course: removes every class group membership, drops module_enrollment rows, resets all gmk_course_progre rows for (user, course) to Disponible, unenrols from the course and deletes the student role. Intended to be the default action of the Retirar button so the user does not have to know which class to withdraw from.',
        'type'        => 'write',
        'capabilities'  => 'local/grupomakro_core:manageacademicstatus',
        'ajax'        => true,
        'services'    => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_reopen_assignment' => array(
        'classname'   => 'local_grupomakro_core\external\teacher\reopen_assignment',
        'methodname'  => 'execute',
        'description' => 'Reopen a student assignment submission so they can submit again',
        'type'        => 'write',
        'capabilities'  => '',
        'ajax'        => true,
        'services'    => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    // Odoo Integration Services
    'local_grupomakro_odoo_enroll_student' => array(
        'classname'   => 'local_grupomakro_core\external\odoo\enroll_student',
        'methodname'  => 'execute',
        'description' => 'Enroll a student into a learning plan based on Odoo Product Name',
        'capabilities'  => 'local/grupomakro_core:manage_debug',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_odoo_unenroll_student' => array(
        'classname'   => 'local_grupomakro_core\external\odoo\unenroll_student',
        'methodname'  => 'execute',
        'description' => 'Unenroll a student from a learning plan based on Odoo Product Name',
        'capabilities'  => 'local/grupomakro_core:manage_debug',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_odoo_update_status' => array(
        'classname'   => 'local_grupomakro_core\external\odoo\update_status',
        'methodname'  => 'execute',
        'description' => 'Update student status (active/suspended) from Odoo',
        'capabilities'  => 'local/grupomakro_core:manage_debug',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_toggle_class_status' => array(
        'classname'   => 'local_grupomakro_core\external\gmkclass\toggle_class_status',
        'methodname'  => 'execute',
        'description' => 'Toggle class closed status and update dates',
        'capabilities'  => 'local/grupomakro_core:manage_classes',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_revert_approval' => array(
        'classname'   => 'local_grupomakro_core\external\schedule\revert_approval',
        'methodname'  => 'execute',
        'description' => 'Revert class approval',
        'capabilities'  => 'local/grupomakro_core:manage_schedules',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_search_users' => array(
        'classname'   => 'local_grupomakro_core\external\user\search_users',
        'methodname'  => 'execute',
        'description' => 'Search users for enrollment',
        'capabilities'  => 'local/grupomakro_core:manage_users',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_gmk_roles' => array(
        'classname'   => 'local_grupomakro_core\external\user\get_gmk_roles',
        'methodname'  => 'execute',
        'description' => 'Return the caller\'s gmk_* roles at system context plus the siteadmin flag. Used by the LXP to skip student-side restrictions for staff.',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => array(MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'),
    ),
    'local_grupomakro_manual_enroll' => array(
        'classname'   => 'local_grupomakro_core\external\schedule\manual_enroll',
        'methodname'  => 'execute',
        'description' => 'Manually enroll user to class',
        'capabilities'  => 'local/grupomakro_core:bulk_enroll',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_sync_progress' => array(
        'classname'   => 'local_grupomakro_core\external\student\sync_progress',
        'methodname'  => 'execute',
        'description' => 'Synchronize progress for all students',
        'capabilities'  => 'local/grupomakro_core:manage_users',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_get_teacher_dashboard_data' => array(
        'classname'   => 'local_grupomakro_core\external\teacher\get_dashboard_data',
        'methodname'  => 'execute',
        'description' => 'Get consolidated data for the teacher dashboard',
        'capabilities'  => '',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_create_express_activity' => array(
        'classname'   => 'local_grupomakro_core\external\teacher\create_express_activity',
        'methodname'  => 'execute',
        'description' => 'Create a Moodle activity with pre-calculated parameters',
        'capabilities'  => '',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_update_teacher_profile' => array(
        'classname'   => 'local_grupomakro_core\external\teacher\update_profile',
        'methodname'  => 'execute',
        'description' => 'Update teacher profile information',
        'capabilities'  => '',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => array(MOODLE_OFFICIAL_MOBILE_SERVICE),
    ),
    'local_grupomakro_change_teacher_password' => [
        'classname' => 'local_grupomakro_core\external\teacher\change_password',
        'methodname' => 'execute',
        'description' => 'Change teacher password',
        'capabilities'  => '',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    // Academic Planning
    'local_grupomakro_get_demand_analysis' => [
        'classname' => 'local_grupomakro_core\external\admin\planning',
        'methodname' => 'get_demand_analysis',
        'description' => 'Get student demand analysis',
        'capabilities'  => 'local/grupomakro_core:manage_academic_planning',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_periods' => [
        'classname' => 'local_grupomakro_core\external\admin\planning',
        'methodname' => 'get_periods',
        'description' => 'Get academic periods',
        'capabilities'  => 'local/grupomakro_core:manage_academic_planning',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_save_period' => [
        'classname' => 'local_grupomakro_core\external\admin\planning',
        'methodname' => 'save_period',
        'description' => 'Save academic period',
        'capabilities'  => 'local/grupomakro_core:manage_academic_planning',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_save_planning' => [
        'classname' => 'local_grupomakro_core\external\admin\planning',
        'methodname' => 'save_planning',
        'description' => 'Save academic planning selections',
        'capabilities'  => 'local/grupomakro_core:manage_academic_planning',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_manually_update_student_period' => [
        'classname' => 'local_grupomakro_core\external\admin\planning',
        'methodname' => 'manually_update_student_period',
        'description' => 'Manually update student period and status',
        'capabilities'  => 'local/grupomakro_core:manage_academic_planning',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    // Scheduler API
    'local_grupomakro_get_scheduler_context' => [
        'classname' => 'local_grupomakro_core\external\admin\scheduler',
        'methodname' => 'get_scheduler_context',
        'description' => 'Get scheduler configuration context',
        'capabilities'  => 'local/grupomakro_core:manage_schedules',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_save_scheduler_config' => [
        'classname' => 'local_grupomakro_core\external\admin\scheduler',
        'methodname' => 'save_scheduler_config',
        'description' => 'Save scheduler configuration',
        'capabilities'  => 'local/grupomakro_core:manage_schedules',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_demand_data' => [
        'classname' => 'local_grupomakro_core\external\admin\scheduler',
        'methodname' => 'get_demand_data',
        'description' => 'Get granular demand data for scheduler',
        'capabilities'  => 'local/grupomakro_core:manage_schedules',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_save_projections' => [
        'classname' => 'local_grupomakro_core\external\admin\scheduler',
        'methodname' => 'save_projections',
        'description' => 'Save manual student projections',
        'capabilities'  => 'local/grupomakro_core:manage_schedules',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_save_generation_result' => [
        'classname' => 'local_grupomakro_core\external\admin\scheduler',
        'methodname' => 'save_generation_result',
        'description' => 'Save generated class schedules',
        'capabilities'  => 'local/grupomakro_core:manage_schedules',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_update_student_status' => [
        // NOTE: the AJAX action 'local_grupomakro_update_student_status' is a
        // separate thing that hits local_grupomakro_sync_financial_status() for a
        // single user (see ajax.php case). The WS REST endpoint below is a
        // status-change action (studentstatus / academicstatus fields).
        // The two are deliberately de-coupled; do NOT rename one to match
        // the other or callers will silently switch behaviour.
        'classname' => 'local_grupomakro_core\external\student\update_student_status',
        'methodname' => 'execute',
        'description' => 'Update student status (general or academic)',
        'capabilities'  => 'local/grupomakro_core:manageacademicstatus',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_update_class_quota' => [
        'classname' => 'local_grupomakro_core\external\schedule\update_class_quota',
        'methodname' => 'execute',
        'description' => 'Update class quota and promote queue students',
        'capabilities'  => 'local/grupomakro_core:manage_schedules',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_letter_get_types' => [
        'classname' => 'local_grupomakro_core\external\letter\get_types',
        'methodname' => 'execute',
        'description' => 'Get active letter types for student requests',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_letter_create_request' => [
        'classname' => 'local_grupomakro_core\external\letter\create_request',
        'methodname' => 'execute',
        'description' => 'Create a new student letter request',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_letter_get_my_requests' => [
        'classname' => 'local_grupomakro_core\external\letter\get_my_requests',
        'methodname' => 'execute',
        'description' => 'Get letter requests created by current student',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_letter_get_request_detail' => [
        'classname' => 'local_grupomakro_core\external\letter\get_request_detail',
        'methodname' => 'execute',
        'description' => 'Get one letter request detail',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_letter_download' => [
        'classname' => 'local_grupomakro_core\external\letter\download',
        'methodname' => 'execute',
        'description' => 'Download generated letter document as base64 payload',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_student_timeline_careers' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'get_careers_list',
        'description' => 'Returns list of careers with active student counts for the timeline view',
        'capabilities'  => 'local/grupomakro_core:view_student_timeline',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_get_student_career_timeline' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'get_career_timeline',
        'description' => 'Returns full timeline data for a career grouped by intake period',
        'capabilities'  => 'local/grupomakro_core:view_student_timeline',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_get_students_by_subperiod' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'get_students_by_subperiod',
        'description' => 'Returns list of students in a specific subperiod with phone and actions',
        'capabilities'  => 'local/grupomakro_core:view_student_timeline',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_reassign_student_intake_period' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'reassign_student_intake_period',
        'description' => 'Reassigns a students intake period (cohort)',
        'capabilities'  => 'local/grupomakro_core:manage_student_timeline',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_get_courses_by_learning_plan' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'get_courses_by_learning_plan',
        'description' => 'Get courses for a learning plan with pending student counts',
        'capabilities'  => 'local/grupomakro_core:view_student_timeline',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_get_courses_with_projections' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'get_courses_with_projections',
        'description' => 'Get courses with projections and semaphore status per cohort/jornada',
        'capabilities'  => 'local/grupomakro_core:view_student_timeline',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_save_course_projection' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'save_course_projection',
        'description' => 'Save or update a course projection (drag-drop result) for a subperiod/jornada',
        'capabilities'  => 'local/grupomakro_core:manage_student_timeline',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_delete_course_projection' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'delete_course_projection',
        'description' => 'Delete a course projection for a subperiod/jornada',
        'capabilities'  => 'local/grupomakro_core:manage_student_timeline',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_get_students_by_intake_period' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'get_students_by_intake_period',
        'description' => 'Returns all students in a given intake period (cohort) grouped by subperiod for bulk reassignment',
        'capabilities'  => 'local/grupomakro_core:view_student_timeline',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_bulk_reassign_students_intake_period' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'bulk_reassign_students_intake_period',
        'description' => 'Bulk reassigns the intake period of multiple students at once (cohort reclassification)',
        'capabilities'  => 'local/grupomakro_core:manage_student_timeline',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_get_students_by_academic_period' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'get_students_by_academic_period',
        'description' => 'Returns students in a learning plan grouped by their current academic period (periodo lectivo) for bulk reclassification',
        'capabilities'  => 'local/grupomakro_core:view_student_timeline',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_bulk_update_students_academic_period' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'bulk_update_students_academic_period',
        'description' => 'Bulk reassigns the academic period (periodo lectivo) of multiple students. Does not touch currentperiodid/currentsubperiodid.',
        'capabilities'  => 'local/grupomakro_core:manage_student_timeline',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_get_period_renewal_preview' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'get_period_renewal_preview',
        'description' => 'Preview of automatic period renewal (B1->B2, B2->nextCuatri/graduando). Inactive stay.',
        'capabilities'  => 'local/grupomakro_core:manage_student_timeline',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_execute_period_renewal' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'execute_period_renewal',
        'description' => 'Executes the period renewal in a single transaction (auto: next bim, next cuatri, graduando)',
        'capabilities'  => 'local/grupomakro_core:manage_student_timeline',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    // --- Malla por grupo (gmk_group_curriculum) ---
    'local_grupomakro_get_group_curriculum' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'get_group_curriculum',
        'description' => 'Returns the declared curriculum of a student group (cohort), grouped by plan level',
        'capabilities'  => 'local/grupomakro_core:view_student_timeline',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_seed_group_curriculum' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'seed_group_curriculum',
        'description' => 'Seeds a group curriculum from the learning plan required courses. Idempotent.',
        'capabilities'  => 'local/grupomakro_core:manage_student_timeline',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_set_group_course' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'set_group_course',
        'description' => 'Adds or updates a course in a group curriculum. Courses outside the plan are flagged source=1.',
        'capabilities'  => 'local/grupomakro_core:manage_student_timeline',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_unset_group_course' => [
        'classname'   => 'local_grupomakro_core\external\admin\student_timeline',
        'methodname'  => 'unset_group_course',
        'description' => 'Removes a course from a group curriculum. Plan courses are flagged excluded (source=2), not deleted.',
        'capabilities'  => 'local/grupomakro_core:manage_student_timeline',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_get_lp_students' => [
        'classname'   => 'local_grupomakro_core\external\lp\get_lp_students',
        'methodname'  => 'execute',
        'description' => 'Get students enrolled in a learning plan, filtered by plan/period/subperiod',
        'capabilities'  => 'local/grupomakro_core:bulk_enroll',
        'type'        => 'read',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    'local_grupomakro_bulk_enroll' => [
        'classname'   => 'local_grupomakro_core\external\lp\bulk_enroll',
        'methodname'  => 'execute',
        'description' => 'Bulk enroll a list of students into a target learning plan',
        'capabilities'  => 'local/grupomakro_core:bulk_enroll',
        'type'        => 'write',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE],
    ],
    // Absence alert system (staged per-class)
    // Student-facing revalidations for the LXP (popup + Reválidas page).
    // Replaces local_grupomakro_student_get_revalids, which reads the
    // abandoned status-7 flow and always returns an empty list.
    'local_grupomakro_get_student_revalidations' => [
        'classname'   => 'local_grupomakro_core\external\student\get_student_revalidations',
        'methodname'  => 'execute',
        'description' => 'Get the revalidations of a student (session, payment link and state).',
        'type'        => 'read',
        'capabilities'=> 'moodle/site:config',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_dismiss_revalidation_alert' => [
        'classname'   => 'local_grupomakro_core\external\student\dismiss_revalidation_alert',
        'methodname'  => 'execute',
        'description' => 'Dismiss the revalidation-scheduled popup for a student.',
        'type'        => 'write',
        'capabilities'=> 'moodle/site:config',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_student_absence_summary' => [
        'classname'   => 'local_grupomakro_core\external\student\get_absence_summary',
        'methodname'  => 'execute',
        'description' => 'Get per-class absence state and the global inactivity flag for a student.',
        'type'        => 'read',
        'capabilities'=> 'moodle/site:config',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_dismiss_absence_alert' => [
        'classname'   => 'local_grupomakro_core\external\student\dismiss_absence_alert',
        'methodname'  => 'execute',
        'description' => 'Persist that a student dismissed the info (1) or warning (2) absence alert for a class.',
        'type'        => 'write',
        'capabilities'=> 'moodle/site:config',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_check_course_access' => [
        'classname'   => 'local_grupomakro_core\external\student\check_course_access',
        'methodname'  => 'execute',
        'description' => 'Check whether the requesting student can access a course; returns the blocking class id when blocked by absence.',
        'type'        => 'read',
        'capabilities'=> '',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_refresh_absence_state' => [
        'classname'   => 'local_grupomakro_core\external\student\refresh_absence_state',
        'methodname'  => 'execute',
        'description' => 'Recompute absence state for the requesting student across their active classes (cron/observer still runs; useful for manual sync).',
        'type'        => 'write',
        'capabilities'=> '',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_course_absences_detail' => [
        'classname'   => 'local_grupomakro_core\external\student\get_course_absences_detail',
        'methodname'  => 'execute',
        'description' => 'Per-class breakdown of absences for a (student, course) pair, including the list of absent/unmarked sessions. Powers the clickable absence chip in the academic panel grades modal.',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:viewabsencedashboard',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    // ── Revalidations Director Dashboard ───────────────────────────────────
    'local_grupomakro_list_revalidations_director' => [
        'classname'   => 'local_grupomakro_core\external\admin\list_revalidations_director',
        'methodname'  => 'execute',
        'description' => 'List revalidation requests for the academic director dashboard with filters and pagination',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_revalidations_dashboard',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_refresh_revalidation_payment' => [
        'classname'   => 'local_grupomakro_core\external\admin\refresh_revalidation_payment',
        'methodname'  => 'execute',
        'description' => 'Refresh the payment state of a single revalidation against Odoo/Express',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:view_revalidations_dashboard',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_refresh_revalidations_bulk' => [
        'classname'   => 'local_grupomakro_core\external\admin\refresh_revalidations_bulk',
        'methodname'  => 'execute',
        'description' => 'Refresh payment state for up to N revalidations against Odoo/Express',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:view_revalidations_dashboard',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_eligible_students_for_extemporaneous' => [
        'classname'   => 'local_grupomakro_core\external\admin\get_eligible_students_for_extemporaneous',
        'methodname'  => 'execute',
        'description' => 'Search students of a class with their revalidation eligibility info, for the extemporaneous wizard',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:create_extemporaneous_revalidations',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_create_extemporaneous_revalidation' => [
        'classname'   => 'local_grupomakro_core\external\admin\create_extemporaneous_revalidation',
        'methodname'  => 'execute',
        'description' => 'Create an extemporaneous revalidation request for a (class, student) pair, bypassing the time window but enforcing eligibility rules',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:create_extemporaneous_revalidations',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_classes_for_search' => [
        'classname'   => 'local_grupomakro_core\external\admin\get_classes_for_search',
        'methodname'  => 'execute',
        'description' => 'Search classes for the extemporaneous revalidation wizard autocomplete (by name, course, instructor)',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:create_extemporaneous_revalidations',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    // ── Failed Subjects Report ────────────────────────────────────────────
    'local_grupomakro_get_failed_subjects_periods' => [
        'classname'   => 'local_grupomakro_core\external\admin\failed_subjects\get_periods',
        'methodname'  => 'execute',
        'description' => 'List academic periods for the failed-subjects report dropdown',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_failed_subjects_report',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_failed_subjects_report' => [
        'classname'   => 'local_grupomakro_core\external\admin\failed_subjects\get_report',
        'methodname'  => 'execute',
        'description' => 'Consolidated failed-subjects report: students with reprobadas/revalidating subjects, matched against projected gmk_class in the selected period by jornada and quota',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_failed_subjects_report',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_failed_subjects_student_detail' => [
        'classname'   => 'local_grupomakro_core\external\admin\failed_subjects\get_student_detail',
        'methodname'  => 'execute',
        'description' => 'Full student detail (contact, plans, history) for the failed-subjects report drawer',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_failed_subjects_report',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_enrol_student_from_failed_subjects' => [
        'classname'   => 'local_grupomakro_core\external\admin\failed_subjects\enrol_student',
        'methodname'  => 'execute',
        'description' => 'Enrol a student in a projected class directly from the failed-subjects report; force_over=true bypasses the quota and logs the action',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:enrol_from_failed_subjects_report',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_clear_failed_subjects_cache' => [
        'classname'   => 'local_grupomakro_core\external\admin\failed_subjects\clear_cache',
        'methodname'  => 'execute',
        'description' => 'Invalidate the per-process failed-subjects report cache',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:view_failed_subjects_report',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_refresh_financial_status_failed_subjects' => [
        'classname'   => 'local_grupomakro_core\external\admin\failed_subjects\refresh_financial_status',
        'methodname'  => 'execute',
        'description' => 'Refresh a single student financial status against Odoo via the Express proxy (same flow as academicpanel refresh button)',
        'type'        => 'write',
        'capabilities'  => 'local/grupomakro_core:enrol_from_failed_subjects_report',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_financial_counts' => [
        'classname'   => 'local_grupomakro_core\external\student\get_financial_counts',
        'methodname'  => 'execute',
        'description' => 'Lightweight count of active students broken down by financial status (al_dia/mora/becado/convenio/sin_contrato/pendiente). Powers the academicpanel 3-card indicator; called by the 60s polling.',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_academic_panel',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    // ── Admin broadcast messages (info/warning to student LXP) ──────────────
    // These notifications take precedence over the absence alert system by
    // default (priority 50 vs. 10). The student-side endpoints are read/write
    // since they accept acknowledgements; the admin-side endpoints are
    // capability-gated.
    'local_grupomakro_get_pending_admin_messages' => [
        'classname'   => 'local_grupomakro_core\external\student\get_pending_admin_messages',
        'methodname'  => 'execute',
        'description' => 'Return admin broadcast messages still pending for the calling student. Sorted by priority DESC so the LXP can take precedence over absence alerts.',
        'type'        => 'read',
        'capabilities'=> '',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_acknowledge_admin_message' => [
        'classname'   => 'local_grupomakro_core\external\student\acknowledge_admin_message',
        'methodname'  => 'execute',
        'description' => 'Record the student acknowledgement (or informational dismiss) of an admin broadcast message',
        'type'        => 'write',
        'capabilities'=> '',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_create_admin_message' => [
        'classname'   => 'local_grupomakro_core\external\admin\announcement\create_message',
        'methodname'  => 'execute',
        'description' => 'Create a new admin broadcast (info or warning). Materialises the audience so per-career stats stay cheap.',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:manageannouncements',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_list_messages' => [
        'classname'   => 'local_grupomakro_core\external\admin\announcement\admin_list_messages',
        'methodname'  => 'execute',
        'description' => 'List existing admin broadcasts with audience and ack counters, plus the careers/groups dropdown catalogues',
        'type'        => 'read',
        'capabilities'  => 'local/grupomakro_core:manageannouncements',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_admin_message_stats' => [
        'classname'   => 'local_grupomakro_core\external\admin\announcement\get_message_stats',
        'methodname'  => 'execute',
        'description' => 'Per-career acknowledgement counters for a single broadcast message',
        'type'        => 'read',
        'capabilities'  => 'local/grupomakro_core:manageannouncements',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_list_admin_message_recipients' => [
        'classname'   => 'local_grupomakro_core\external\admin\announcement\list_message_recipients',
        'methodname'  => 'execute',
        'description' => 'Flat list of recipients of a broadcast, with their individual ack state, for the admin dashboard drawer',
        'type'        => 'read',
        'capabilities'  => 'local/grupomakro_core:manageannouncements',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_set_admin_message_active' => [
        'classname'   => 'local_grupomakro_core\external\admin\announcement\set_message_active',
        'methodname'  => 'execute',
        'description' => 'Enable or disable an existing broadcast (soft delete)',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:manageannouncements',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_revert_homologation' => [
        'classname'   => 'local_grupomakro_core\external\student\revert_homologation',
        'methodname'  => 'execute',
        'description' => 'Revert a previously applied homologation: clears the homologation fields on gmk_course_progre, resets status to Disponible, and nulls the Nota Final Integrada / course total grade_grades rows.',
        'type'        => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_homologations',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_homologation_audit' => [
        'classname'   => 'local_grupomakro_core\external\student\get_homologation_audit',
        'methodname'  => 'execute',
        'description' => 'Return the chronological list of homologate/revert actions for a (student, course, plan) triple, sourced from gmk_homologation_audit. Powers the timeline dialog in the academic panel grades modal.',
        'type'        => 'read',
        'capabilities'  => 'local/grupomakro_core:view_movement_audit',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_list_classes_paged' => [
        'classname'   => 'local_grupomakro_core\external\gmkclass\list_classes_paged',
        'methodname'  => 'execute',
        'description' => 'Server-side paginated listing of classes for the classmanagement page: search across name/shift/instructor/learning plan/course, filters by period/learning plan/core course, status (active/closed/all), sort + dir. Reuses list_classes() bulk-prefetch.',
        'type'        => 'read',
        'capabilities'  => 'local/grupomakro_core:view_classmanagement',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_status_change_preview' => [
        'classname'   => 'local_grupomakro_core\external\student\status_change_preview',
        'methodname'  => 'execute',
        'description' => 'Build the preview payload for the LXP wizard (academicpanel): student profile, per-plan courses, Odoo pending invoices (best-effort) and the dropdown of gmk_academic_periods to suggest as reactivation target.',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:manageacademicstatus',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_execute_status_change' => [
        'classname'   => 'local_grupomakro_core\external\student\status_change_execute',
        'methodname'  => 'execute',
        'description' => 'Apply a postponement (aplazar) or withdrawal (retirar) to a student: drops active courses, flips status on local_learning_users + user_info_data, writes gmk_student_suspension with origin=lxp and dispatches the same effect to Odoo through the Express proxy.',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:manageacademicstatus',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_status_change_history' => [
        'classname'   => 'local_grupomakro_core\external\student\status_change_history',
        'methodname'  => 'execute',
        'description' => 'Return the chronological list of postponement/withdrawal/renovation rows from gmk_student_suspension for a user, formatted to feed the v-timeline in grademodal.js.',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:manageacademicstatus',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],

    // ── Wellness module — public / student ──────────────────────────────────
    'local_grupomakro_get_wellness_partners' => [
        'classname'   => 'local_grupomakro_core\external\wellness\get_partners',
        'methodname'  => 'execute',
        'description' => 'Public catalogue of wellness partners (RF-01). Filters by keyword and category.',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_wellness_events' => [
        'classname'   => 'local_grupomakro_core\external\wellness\get_events',
        'methodname'  => 'execute',
        'description' => 'Public list of wellness events (RF-02/04/05). Honours visibility and registration windows.',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_wellness_event_detail' => [
        'classname'   => 'local_grupomakro_core\external\wellness\get_event_detail',
        'methodname'  => 'execute',
        'description' => 'Detail of a single wellness event with attachments and registration counts (RF-04).',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_wellness_calendar_month' => [
        'classname'   => 'local_grupomakro_core\external\wellness\get_calendar_month',
        'methodname'  => 'execute',
        'description' => 'Events intersecting a given (year, month) for the wellness calendar (RF-05).',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_register_wellness_event' => [
        'classname'   => 'local_grupomakro_core\external\wellness\register_event',
        'methodname'  => 'execute',
        'description' => 'Register the calling student to a wellness event. Honours capacity and waitlist (RF-02.2/02.3).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_cancel_wellness_registration' => [
        'classname'   => 'local_grupomakro_core\external\wellness\cancel_registration',
        'methodname'  => 'execute',
        'description' => 'Cancel a wellness event registration; promotes waitlist head if applicable (RF-02).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_my_wellness_registrations' => [
        'classname'   => 'local_grupomakro_core\external\wellness\get_my_registrations',
        'methodname'  => 'execute',
        'description' => 'List the wellness event registrations of the calling student.',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_wellness_dynamic_form' => [
        'classname'   => 'local_grupomakro_core\external\wellness\get_dynamic_form',
        'methodname'  => 'execute',
        'description' => 'Read the active dynamic form attached to an event (RF-06).',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_submit_wellness_dynamic_form' => [
        'classname'   => 'local_grupomakro_core\external\wellness\submit_dynamic_form',
        'methodname'  => 'execute',
        'description' => 'Submit a student answers to a dynamic form (RF-06).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],

    // ── Wellness module — admin / back-office ───────────────────────────────
    'local_grupomakro_admin_list_wellness_partners' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_list_partners',
        'methodname'  => 'execute',
        'description' => 'Admin list of every wellness partner (active + inactive) + category catalogue (RF-09.1).',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_save_wellness_partner' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_save_partner',
        'methodname'  => 'execute',
        'description' => 'Admin upsert a wellness partner (RF-09.1).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_toggle_wellness_partner_active' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_toggle_partner_active',
        'methodname'  => 'execute',
        'description' => 'Soft-delete / restore a wellness partner (RF-09.1).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_list_wellness_events' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_list_events',
        'methodname'  => 'execute',
        'description' => 'Admin list of every wellness event with registration counts (RF-09.2).',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_save_wellness_event' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_save_event',
        'methodname'  => 'execute',
        'description' => 'Admin upsert a wellness event with attachments (RF-09.2).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_toggle_wellness_event_active' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_toggle_event_active',
        'methodname'  => 'execute',
        'description' => 'Soft-delete / restore a wellness event (RF-09.2).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_delete_wellness_event' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_delete_wellness_event',
        'methodname'  => 'execute',
        'description' => 'Hard-delete a wellness event + its registrations, files and BBB room (RF-09.2).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_export_event_registrations' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_export_event_registrations',
        'methodname'  => 'execute',
        'description' => 'Export registrations of an event as CSV (RF-09.2).',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_list_event_registrations' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_list_event_registrations',
        'methodname'  => 'execute',
        'description' => 'List registrations of an event with student details (RF-02 / RF-09.2).',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_cancel_event_registration' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_cancel_event_registration',
        'methodname'  => 'execute',
        'description' => 'Admin: cancel a specific student\'s registration to an event (RF-02 / RF-09.2).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_create_wellness_event_bbb' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_create_wellness_event_bbb',
        'methodname'  => 'execute',
        'description' => 'Provision a BigBlueButton room for a wellness event and expose its guest URL (RF-04).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_delete_wellness_event_bbb' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_delete_wellness_event_bbb',
        'methodname'  => 'execute',
        'description' => 'Tear down the BigBlueButton room attached to a wellness event (RF-04).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_list_wellness_dynamic_forms' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_list_dynamic_forms',
        'methodname'  => 'execute',
        'description' => 'Admin list of every dynamic form (RF-06/09.2).',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_save_wellness_dynamic_form' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_save_dynamic_form',
        'methodname'  => 'execute',
        'description' => 'Admin upsert a wellness dynamic form (RF-06).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_toggle_wellness_dynamic_form_active' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_toggle_dynamic_form_active',
        'methodname'  => 'execute',
        'description' => 'Toggle active flag on a wellness dynamic form (RF-06).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_list_wellness_dynamic_form_responses' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_list_dynamic_form_responses',
        'methodname'  => 'execute',
        'description' => 'List submitted responses for a dynamic form (RF-06).',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],

    // ── Wellness psychology — student (RF-03) ────────────────────────────────
    'local_grupomakro_get_psychology_slots' => [
        'classname'   => 'local_grupomakro_core\external\wellness\get_psychology_slots',
        'methodname'  => 'execute',
        'description' => 'Open psychology slot occurrences within a date range (RF-03.1).',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_request_psychology_appointment' => [
        'classname'   => 'local_grupomakro_core\external\wellness\request_psychology_appointment',
        'methodname'  => 'execute',
        'description' => 'Book a psychology appointment for the calling student; fires the staff + student notifications (RF-03.2/03.3/03.4).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_get_my_psychology_appointments' => [
        'classname'   => 'local_grupomakro_core\external\wellness\get_my_psychology_appointments',
        'methodname'  => 'execute',
        'description' => 'Calling student\'s psychology appointments (RF-03 / RF-09.3 view).',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],

    // ── Wellness psychology — admin (RF-09.3) ──────────────────────────────
    'local_grupomakro_admin_list_psychology_appointments' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_list_psychology_appointments',
        'methodname'  => 'execute',
        'description' => 'Admin agenda view for psychology appointments (RF-09.3).',
        'type'        => 'read',
        'capabilities'  => 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_update_psychology_appointment' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_update_psychology_appointment',
        'methodname'  => 'execute',
        'description' => 'Apply a status transition (confirmar|modificar|cancelar|atendida|no_asistio) with email side-effects (RF-09.3).',
        'type'        => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_save_psychology_slots' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_save_psychology_slots',
        'methodname'  => 'execute',
        'description' => 'Admin CRUD for the recurring psychology schedule slots (RF-03.1).',
        'type'        => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],

    // ── Wellness staff roster (RF-03, RF-09.3 — editable personnel) ────────
    'local_grupomakro_admin_list_staff' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_list_staff',
        'methodname'  => 'execute',
        'description' => 'Admin list of the editable wellness staff roster (psicólogo titular/suplente, Talento Humano, Bienestar Estudiantil).',
        'type'        => 'read',
        'capabilities'  => 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_upsert_staff' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_upsert_staff',
        'methodname'  => 'execute',
        'description' => 'Admin upsert a wellness staff role with email override; changes are audited in gmk_wellness_staff_audit.',
        'type'        => 'write',
        'capabilities'  => 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_staff_history' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_staff_history',
        'methodname'  => 'execute',
        'description' => 'Audit trail of changes to gmk_wellness_staff_role.',
        'type'        => 'read',
        'capabilities'  => 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],

    // ── Wellness carnet (RF-07, RF-09.4) ─────────────────────────────────────
    'local_grupomakro_get_wellness_carnet' => [
        'classname'   => 'local_grupomakro_core\external\wellness\get_carnet',
        'methodname'  => 'execute',
        'description' => 'Return (and lazily issue) the calling student\'s digital carnet + QR URL (RF-07).',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    // -- Evaluacion docente post-sesion (RF-08, Fase 4) ----------------------
    'local_grupomakro_get_pending_teacher_evals' => [
        'classname'   => 'local_grupomakro_core\external\wellness\get_pending_teacher_evals',
        'methodname'  => 'execute',
        'description' => 'Sesiones que el estudiante todavia puede evaluar (RF-08).',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_core_get_wellness_eval_enabled' => [
        'classname'   => 'local_grupomakro_core\external\wellness\get_wellness_eval_enabled',
        'methodname'  => 'execute',
        'description' => 'Configuracion del lado Moodle para el popup de evaluacion docente (RF-08).',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_submit_teacher_eval' => [
        'classname'   => 'local_grupomakro_core\external\wellness\submit_teacher_eval',
        'methodname'  => 'execute',
        'description' => 'Guarda la evaluacion docente de una sesion (RF-08).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_dismiss_teacher_eval' => [
        'classname'   => 'local_grupomakro_core\external\wellness\dismiss_teacher_eval',
        'methodname'  => 'execute',
        'description' => 'Descarta el popup de evaluacion de una sesion (RF-08).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:view_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_list_teacher_evals' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_list_teacher_evals',
        'methodname'  => 'execute',
        'description' => 'Resultados y promedios de evaluacion docente (RF-08).',
        'type'        => 'read',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_upload_wellness_image' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_upload_wellness_image',
        'methodname'  => 'execute',
        'description' => 'Sube la imagen de portada de un convenio, evento o formulario dinamico y guarda su URL.',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
    'local_grupomakro_admin_manage_carnet' => [
        'classname'   => 'local_grupomakro_core\external\admin\wellness\admin_manage_carnet',
        'methodname'  => 'execute',
        'description' => 'Admin operations on a student carnet: renew, suspend, reinstate, regenerate_token, graduate (RF-07/09.4).',
        'type'        => 'write',
        'capabilities'=> 'local/grupomakro_core:manage_wellness',
        'ajax'        => true,
        'services'    => [MOODLE_OFFICIAL_MOBILE_SERVICE, 'local_grupomakro_core'],
    ],
);
