...
// Get the admin user or a specific user
$adminuser = get_admin(); // Get admin user
...
$enableStudentsCreation = false;
$enableTeachersCreation = false;
$enableLPCreation = false;
$enableCourseCreation = false;
$enableCourseLPCreation = false;
$enableStudentLPCreation = false;
$enableTeacherLPCreation = false;
$enableClassCreation = true;
...
$classCreationResults = ['success' => [], 'error' => []];
    ...
    for ($row = 1; $row <= $highestRow; $row++) {
        ...
        if ($sheetName === "Estudiante-basico") {
            ...
            try {
                ...
                if ($limit !== 0 && $createdStudents >= $limit) {
                ...
                $creationResult['Error'] = property_exists($e, 'debuginfo') ? $e->debuginfo : $e->getMessage();
        ...
        if ($sheetName === "Docente-basico") {
            ...
            try {
                ...
                if ($limit !== 0 && $createdTeachers >= $limit) {
                ...
                create_teacher_disponibility($userName);
                $creationResult['Id asignado'] = $createdTeacher['id'];
                $creationResult['Mensaje'] = $message;
                $teachersCreationResults['success'][$rowData[2]] = $creationResult;
                $createdTeachers += 1;
            } catch (Exception $e) {
                $DB->force_transaction_rollback();
                $creationResult['Error'] = property_exists($e, 'debuginfo') ? $e->debuginfo : $e->getMessage();
                $teachersCreationResults['error'][$rowData[2]] = $creationResult;
                $createdTeachers += 1;
            }
        }
        ...
        if ($sheetName === "Curso-basico") {
            ...
            try {
                ...
                // throw new Exception('Creacion de LP deshabilitada.');
                if ($limit !== 0 && $createdCourses >= $limit) {
                ...
                $teacherSkillData = construct_teacher_skill($courseData);
                $DB->insert_record('gmk_teacher_skill', $teacherSkillData);
        ...
        if ($sheetName === "Curso-carrera") {
            ...
            try {
                ...
                if ($limit !== 0 && $courseLPCreated >= $limit) {
                ...
                if (array_key_exists($learningPlanCode, $learningPlanCreationResults['success'])) {
                    $learningPlanShortName = $learningPlanCreationResults['success'][$learningPlanCode]['Nombre corto'];
                } else if (array_key_exists($learningPlanCode, $learningPlanCreationResults['error'])) {
        ...
        if ($sheetName === "Docente-carrera") {
            ...
            $learningPlanCode = $rowData[2];
            $creationResult = ['Nombre de usuario' => $teacherUsername, 'Codigo Carrera' => $learningPlanCode, 'Tabla' => $sheetName, 'Fila' => $row];
            try {
                ...
                // throw new Exception('Creacion de LP deshabilitada.');
                if ($limit !== 0 && $teacherLPCreated >= $limit) {
                    throw new Exception('Se supero el limite de estudiantes LP creados para esta prueba.');
                }
        ...
        if ($sheetName === "Clase") {
            $className = $rowData[1];
            $learningPlanCode = $rowData[3];
            $creationResult = ['Nombre de clase' => $className, 'Codigo Carrera' => $learningPlanCode, 'Tabla' => $sheetName, 'Fila' => $row];
            try {
                if (!$enableClassCreation) {
                    throw new Exception('Creacion de clases deshabilitada.');
                }
                // throw new Exception('Creacion de LP deshabilitada.');
                if ($limit !== 0 && $classesCreated >= $limit) {
                    throw new Exception('Se supero el limite de clases creadas para esta prueba.');
                }
                ...
                $classEntity = construct_class_entity($rowData, $learningPlanId);
                extract($classEntity, EXTR_PREFIX_ALL, 'class');
                $classCreated = \local_grupomakro_core\external\gmkclass\create_class::execute(
                    $class_name,
                    $class_type,
                    $class_learningPlanId,
                    $class_periodId,
                    $class_courseId,
                    $class_instructorId,
                    $class_initTime,
                ...
                if ($classCreated['status'] == '-1') {
                    throw new Exception($classCreated['message']);
                }
                $creationResult['Mensaje'] = 'ok';
                $creationResult['ID clase'] = $classCreated['status'];
                $classCreationResults['success'][] = $creationResult;
                $classesCreated += 1;
            } catch (Exception $e) {
                $creationResult['Error'] = $e->getMessage();
                $classCreationResults['error'][] = $creationResult;
                $classesCreated += 1;
            }
        }
    }
...
$classCreationResults = json_encode($classCreationResults, JSON_PRETTY_PRINT);
...
$classCreationResultsPath = $folderPath . 'classCreationResults.txt';
...
$resultsArray = [
    ...
    $classCreationResultsPath => $classCreationResults,
];
...
function create_teacher_disponibility($userName)
{
    global $DB;

    $teacherId = $DB->get_field('user', 'id', ['username' => $userName]);
    $instructorId = $teacherId;
    $newDisponibilityRecords = [
        [
            'day' => 'Lunes',
    ...
    $skills = [];
    $disponibilityCreated = \local_grupomakro_core\external\disponibility\add_teacher_disponibility::execute($instructorId, $newDisponibilityRecords, $skills);
}

function construct_class_entity($data, $learningPlanId)
{
    global $DB;
    $classTypeDict = [
        'mixta' => 2,
    ...
    $classCourseShortname = $data[4];
    $courseId = $DB->get_field('course', 'id', ['shortname' => $classCourseShortname]);
    if (!$lpCourse = $DB->get_record('local_learning_courses', ['courseid' => $courseId, 'learningplanid' => $learningPlanId], 'id, periodid')) {
        throw new Exception('El curso del LP no existe');
    }
    if (!$teacherId = $DB->get_field('user', 'id', ['username' => $data[8]])) {
        throw new Exception('El docente no existe');
    }
    // if(!$courseSkillId = $DB->get_field('gmk_teacher_skill','id',['shortname'=>$classCourseShortname])){
    //     throw new Exception('El skill no existe');
    // }
    // if($DB->record_exists('gmk_teacher_skill_relation',['skillid'=>$courseSkillId,'userid'=>$teacherId])){
    //     throw new Exception('La relacion de skill ya existe');
    // }
    // $teacherSkillRelation = new stdClass();
    // $teacherSkillRelation->skillid = $courseSkillId;
    // $teacherSkillRelation->userid = $teacherId;

    // var_dump($DB->insert_record('gmk_teacher_skill_relation',$teacherSkillRelation));

    $classEntity = [];
    $classEntity['name'] = $data[1];
    $classEntity['type'] = $classTypeDict[$data[2]];
    $classEntity['learningPlanId'] = (int)$learningPlanId;

    $classEntity['periodId'] = (int)$lpCourse->periodid;
    $classEntity['courseId'] = (int)$lpCourse->id;
    $classEntity['instructorId'] = (int)$teacherId;

    $classEntity['initTime'] = convertHourFormat($data[5]);
    $classEntity['endTime'] = convertHourFormat($data[6]);
    $classEntity['classDays'] = buildDayString($data[7]);

    return $classEntity;
}
function buildDayString($givenDay)
{
    // Days of the week in Spanish starting from Monday
    $daysOfWeek = ["lunes", "martes", "miercoles", "jueves", "viernes", "sabado", "domingo"];

    // Initialize an array with 0s
    $dayStringArray = array_fill(0, 7, '0');

    // Find the index of the given day
    $dayIndex = array_search(strtolower($givenDay), $daysOfWeek);

    // If the given day is found, set the corresponding position to 1
    if ($dayIndex !== false) {
        $dayStringArray[$dayIndex] = '1';
    }

    // Join the array elements with '/'
    return implode('/', $dayStringArray);
}
function convertHourFormat($timeStr)
{
    $hours = floor($timeStr * 24);
    $minutes = round(($timeStr * 24 * 60) % 60);
    $formattedTime = sprintf('%02d:%02d', $hours, $minutes);

    $parts = explode(':', $formattedTime);
    $hour = str_pad($parts[0], 2, '0', STR_PAD_LEFT);  // Pads the hour with a leading zero if necessary
    $minute = $parts[1];
    return "$hour:$minute";
}
...
function construct_student_entity($data)
{
    ...
    var_dump(excelDateToUnixTimestamp($data[9]));
    $studentEntity = [
        'createpassword' => 1,
    ...
    $customFields = [
        ...
        [
            'type' => 'birthdate',
            'value' => excelDateToUnixTimestamp($data[9])
        ],
        [
            'type' => 'studentstatus',
            'value' => $data[13]
...
function construct_teacher_entity($data)
{
    ...
    $customFields = [
        ...
        [
            'type' => 'birthdate',
            'value' => excelDateToUnixTimestamp($data[8])
        ],
        [
            'type' => 'studentstatus',
            'value' => $data[10]
...
function construct_teacher_skill($data)
{
    $teacherSkill = new stdClass();
    $teacherSkill->name = $data['fullname'];
    $teacherSkill->shortname = $data['shortname'];
    return $teacherSkill;
}
...
{
    // Excel's epoch (start date): January 1, 1900
    $excelEpoch = new DateTime('1899-12-30'); // Adjusted for Excel's epoch

    // Excel incorrectly treats 1900 as a leap year
    // We need to add one extra day for dates after February 28, 1900
    $days = floor($excelDate);
    if ($days > 59) { // 59 is the serial number for February 28, 1900
        $days++;
    }

    // Add the number of days (Excel date serial number)
    $interval = new DateInterval('P' . $days . 'D');
    $excelEpoch->add($interval);

    // Return the Unix timestamp
    return $excelEpoch->getTimestamp() - 86400;
}
