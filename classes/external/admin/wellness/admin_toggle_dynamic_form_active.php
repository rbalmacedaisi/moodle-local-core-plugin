...
/**
 * Admin: toggle a dynamic form's active flag (RF-06 / RF-09.2).
 *
 * Active = 1 means the form is exposed to the LXP; active = 0 hides it
 * without deleting the row (existing student responses are preserved).
 *
...
class admin_toggle_dynamic_form_active extends external_api {
    ...
    public static function execute($id, $active) {
        ...
        $ok = \local_grupomakro_core\local\wellness_dynamic_form_manager::set_active(
            (int)$params['id'], $params['active'] ? 1 : 0
        );
        return ['ok' => (bool)$ok];
    }
